...
int checkAdventurer(int curr_player, struct gameState *game){
  ...
  if (newCards == 2 && drawn == 1){
    int secondToLastCard = testGame.hand[curr_player][testGame.handCount[curr_player]-1];
    int lastCard = testGame.hand[curr_player][handpos];
    assertEqual(lastCard, drawnCards[1], &failures);
    printf("last card =%d, expected = %d\n", lastCard, drawnCards[1]);
    // printf("last card =%d, %d, expected = %d\n", testGame.hand[curr_player][testGame.handCount[curr_player]-2], testGame.hand[curr_player][0], drawnCards[1]);
